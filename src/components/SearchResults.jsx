        ...
        <div className="grid grid-cols-2 gap-x-3 gap-y-3 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 lg:gap-5 px-4">
            {
                dados.map((camisa) => (
                    <div
                        key={camisa.id}
                        className="flex flex-col bg-white border rounded-xl p-2 items-center gap-2 shadow-xs w-[167px] h-[260px] md:w-[200px] md:h-[290px] lg:w-[220px] lg:h-[310px] hover:shadow-lg place-items-center relative"
                    >
                        <Link to={`/produto/${camisa.id}`}>
                            ...
                            <div className="bg-primary text-white text-center p-2 font-semibold rounded-xl w-full text-sm hover:scale-[103%]">
                                VER MAIS
                            </div>
                            {camisa.novo && (
